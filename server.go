...
type Option struct {
	...
	ConnectTimeout time.Duration // For Timeout, 0 means no limit
	HandleTimeout  time.Duration // For Timeout
}
...
var DefaultOption = &Option{
	...
	ConnectTimeout: time.Second * 10, // set up the default connection timeout to 10 seconds
}
...
func (server *Server) readRequest(cc codec.Codec) (*request, error) {
	...
	// check if it is pointer, if it is not pointer, then make it as pointer
	// because ReadBody needs a pointer's interface
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	defer wg.Done()
	// handle timeouts when handleResult including 3 cases:
	// 1) when receive request
	// 2) when call services
	// 3) when send request
	// here use called to indicate request has been received, and begin to call sendRequest
	// use sent to indicate the sendRequest is done
...
// Register registers new service to DefaultServer
